...
/** Sub-component that fits the map bounds to the route whenever waypoints change. */
...
export function StreamMap({
  ...
  const positions: L.LatLngExpression[] = waypoints.map((w) => [w.lat, w.lng]);

  // Build spread post markers
  const postInfos: PostMarkerInfo[] = posts
    .filter(isStatusPost)
    .filter(
      (p): p is StatusPost & { location: NonNullable<StatusPost["location"]> } =>
        p.location != null
    )
    .map((p) => ({ post: p, lat: p.location.lat, lng: p.location.lng }));
  const spreadPosts = spreadOverlappingPosts(postInfos);
  ...
  const defaultCenter: L.LatLngExpression =
    waypoints.length > 0 ? [waypoints[0].lat, waypoints[0].lng] : [0, 0];

  const popupImageUrl = popupState?.post.imagePath
      ...
      >
        ...
        {positions.length > 1 && (
          <>
            <Polyline
              positions={positions}
              pathOptions={{ color: "#e53935", weight: 4, opacity: 0.9 }}
            />
        ...
        {trackerPosition && (
          <Marker
            position={[trackerPosition.lat, trackerPosition.lng]}
            icon={trackerIcon}
          />
        )}

        {spreadPosts.map(({ post, lat, lng }) => {
          ...
          return (
            <Marker
              key={`${post.createdAt}-${post.userId}`}
              position={[lat, lng]}
              icon={icon}
              eventHandlers={{
                click: () => setPopupState({ post, lightboxUrl: null }),
              }}
            />
          );
        })}
